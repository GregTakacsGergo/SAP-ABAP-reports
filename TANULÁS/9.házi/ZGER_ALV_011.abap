...
*&---------------------------------------------------------------------*
*& This report analyzes and displays customer data (from KNA1 and KNVV tables) in ALV format. 
*& There is an option to filter by country, customer code, name, postal code, and sales channel. 
*& The report calculates the number of sales channels for each customer and colors the rows based on the values. 
*& Additionally, it displays detailed sales channel data of the customer in a pop-up window when a row is double-clicked.
*&---------------------------------------------------------------------*
REPORT ZGER_ALV_011_HOME_S01E09.

"INCLUDE ZGER_ALV_011_TOP.
"INCLUDE ZGER_ALV_011_SCRN.
TYPE-POOLS: SLIS.
TABLES: KNA1, KNVV, T005T.

DATA GT_FIELDCAT TYPE SLIS_T_FIELDCAT_ALV.

DATA GT_OUTTAB TYPE TABLE OF ZGER_011_HOME09_1_COLOR.
DATA GT_OUTTAB2 TYPE TABLE OF ZGER_011_HOME09_2.
DATA LS_OUTTAB TYPE ZGER_011_HOME09_1_COLOR.
...
SELECTION-SCREEN BEGIN OF BLOCK se2 WITH FRAME TITLE TEXT-002.
  PARAMETERS: P_SZINEZ AS CHECKBOX DEFAULT '',
              P_KULCS AS CHECKBOX DEFAULT '',
              P_GOMB AS CHECKBOX DEFAULT ''.
SELECTION-SCREEN END OF BLOCK se2.

INITIALIZATION. "mezőkatalógus inicializálása.
  G_REPID = SY-REPID.
"INCLUDE zger_alv_011_home_s01e09_exf01.
...
FORM FIELDCAT_INIT USING XT_FIELDCAT TYPE SLIS_T_FIELDCAT_ALV.
    ...
    LOOP AT XT_FIELDCAT INTO XS_FIELDCAT.
      CASE XS_FIELDCAT-FIELDNAME.
        ...
        WHEN 'VEVO_NR'.
          XS_FIELDCAT-seltext_s = 'Szló'.
          XS_FIELDCAT-seltext_m = 'Számláló'.
          XS_FIELDCAT-seltext_l = 'Számláló'.
          XS_FIELDCAT-reptext_ddic = 'Számláló'.
          MODIFY XT_FIELDCAT FROM XS_FIELDCAT INDEX sy-tabix.
        WHEN 'VKORG_NR'.
          XS_FIELDCAT-seltext_s = 'Cs.sz'.
          XS_FIELDCAT-seltext_m = 'Ért. csat. sz'.
          XS_FIELDCAT-seltext_l = 'Értékesítési csat. száma'.
          XS_FIELDCAT-reptext_ddic = 'Értékesítési csatornák száma'.
          XS_FIELDCAT-emphasize  = 'C510'.
          MODIFY XT_FIELDCAT FROM XS_FIELDCAT INDEX sy-tabix.
        WHEN 'KUNNR'.
          XS_FIELDCAT-key = P_KULCS.
          MODIFY XT_FIELDCAT FROM XS_FIELDCAT INDEX sy-tabix.
      ENDCASE.
    ENDLOOP.
...
FORM USER_COMMAND USING R_UCOMM LIKE SY-UCOMM
  ...
  CASE R_UCOMM.
    WHEN '&IC1'.  "duplaklikk
      IF RS_SELFIELD-FIELDNAME EQ 'VKORG_NR'.
        READ TABLE GT_OUTTAB INTO LS_OUTTAB INDEX RS_SELFIELD-TABINDEX.
        IF SY-SUBRC = 0.
          DATA: XT_POPUP TYPE TABLE OF ZGER_011_HOME09_2,
                XS_POPUP TYPE ZGER_011_HOME09_2.

          SELECT KNVV~KUNNR NAME1 VKORG VTWEG SPART
          INTO TABLE XT_POPUP
          FROM  KNVV
          INNER JOIN KNA1 ON KNVV~KUNNR = KNA1~KUNNR
          WHERE KNVV~KUNNR = LS_OUTTAB-KUNNR.

            CALL FUNCTION 'REUSE_ALV_POPUP_TO_SELECT'
              EXPORTING
                I_TITLE = 'Értékesítési csatornák'
                I_TABNAME = 'XT_POPUP'
                I_STRUCTURE_NAME = 'ZGER_011_HOME09_2'
              TABLES
                T_OUTTAB = XT_POPUP
              EXCEPTIONS
                program_error = 1
